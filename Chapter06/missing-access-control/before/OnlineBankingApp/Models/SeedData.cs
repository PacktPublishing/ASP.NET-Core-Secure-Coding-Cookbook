...
namespace OnlineBankingApp.Models
{
    public static class SeedData
    {
        public static async Task Initialize(IServiceProvider serviceProvider)
        {
            ...
            if (!await roleManager.RoleExistsAsync(Role.ActiveCustomer))  
            {  
                await roleManager.CreateAsync(new IdentityRole(Role.ActiveCustomer));
            }
            if (!await roleManager.RoleExistsAsync(Role.PendingCustomer))  
            {  
                await roleManager.CreateAsync(new IdentityRole(Role.PendingCustomer));
            }
            if (!await roleManager.RoleExistsAsync(Role.SuspendedCustomer))  
            {  
                await roleManager.CreateAsync(new IdentityRole(Role.SuspendedCustomer));
            }
            ...
            if (user2 == null)
            {
                ...
                await userManager.AddToRoleAsync(user1,Role.PendingCustomer);
            }
